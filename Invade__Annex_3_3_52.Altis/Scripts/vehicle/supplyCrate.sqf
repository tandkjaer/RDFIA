...
spawnCrateCode = {
	...
	_fillCrate = {
    	...
    	private _light = "#lightpoint" createVehicle (position _crate);
	...
	if (_spawnPos select 2 > 3) then {
	    private _chute = createVehicle ["B_Parachute_02_F", [_spawnPos select 0, _spawnPos select 1, ((_spawnPos select 2) - 5 )], [], 0, 'FLY'];
    	_crate = createVehicle ["B_supplyCrate_F", position _chute, [], 0, 'NONE'];
    	_crate attachTo [_chute, [0, 0, 0]];
    	_crate allowDamage false;
    	private _smoke = createVehicle ["SmokeShellOrange", position _crate, [], 0, 'NONE'];
        ...
        _crate = createVehicle ["B_supplyCrate_F", position _heli, [], 10, 'NONE'];
        _crate allowDamage false;
        [_crate] spawn _fillCrate;
	};
    private _smoke = createVehicle ["SmokeShellOrange", position _crate, [], 0, 'NONE'];
...
[_heli,"drop supplycrate",
"\a3\ui_f\data\IGUI\Cfg\simpleTasks\types\use_ca.paa",
"\a3\ui_f\data\IGUI\Cfg\simpleTasks\types\use_ca.paa",
"(((getPosATL (vehicle _this)) select 2 > 60) || ((getPosATL (vehicle _this)) select 2 < 2)) && (_this == driver (vehicle _this)) && (vehicle _this != _this)",
"(((getPosATL (vehicle _this)) select 2 > 60) || ((getPosATL (vehicle _this)) select 2 < 2)) && (vehicle _this != _this)",
{ hint "Preparing to drop crate";},{},{
	private _heli = _this select 0;
	private _pos = getPos _heli;
	[_pos, _heli] spawn spawnCrateCode; 
	hint "Crate dropped";
},{hint "You stopped preparing to drop the crate";},[], 1, 0,true,false] remoteExecCall ["BIS_fnc_holdActionAdd", 0, true];
