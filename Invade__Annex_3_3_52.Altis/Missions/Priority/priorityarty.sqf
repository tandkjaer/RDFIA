...
private ["_position","_PTdir","_unitsArray","_priorityGroup","_distance","_dir","_c","_pos","_barrier","_enemiesArray","_unit","_targetPos","_fuzzyPos","_briefing","_completeText","_targetUnit"];
...
//-------------------- 5. SPAWN FORCE PROTECTION
	...
	for "_x" from 1 to 5 do {
		...
		[_infantryGroup, _infPos, 300 / 1.6] call BIS_FNC_taskPatrol;
...
//-------------------- setup no firing zone:

if (isNil "artyNoFiringLocations") then {
	artyNoFiringLocations = []; 
};
...
while {canFire priorityObj1 || canFire priorityObj2} do {
			...
			if ( !(isNull _targetUnit) && (vehicle _targetUnit == _targetUnit) && (side _targetUnit == WEST)) then {
				...
				
				{
					...
					if (_x isEqualType []) then {
						_distance = _targetPos distance _x;
					};
					if (_x isEqualType "") then {
						_distance = _targetPos distance (getMarkerPos _x);
					};
					
					if (_distance < 1000) exitWith {_farEnoughFromBase = false;};
				} forEach artyNoFiringLocations;
	...
	private _MaxSleepTime = abs (_ArtilleryTargetTickTimeMax - _ArtilleryTargetTickTimeMin);
	
	while {_TimeSlept < _ArtilleryTargetTickTimeMin} do {
