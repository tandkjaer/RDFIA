...
private _noSpawningRange = missionsMinimumSpawnDistance;
...
if (_cacheBuildingArrayAmount == 0) exitWith {
    diag_log("Error: Side mission destroy urban: Couldn't find any buildings near " + str _RandomTownPosition);
};
...
if (!_accepted) exitWith {
    diag_log("Error: Side mission destroy urban: None of the near buildings had suitable spawn positions in " + str _RandomTownPosition);
};
...
//-------- SPAWN GUARDS and CIVILIANS
	...
	for "_i" from 0 to (2 + (random 2)) do {
		_randomPos = [getPos sideObj, 10, 300, 5, 0, 0.3, 0, [], (getPos sideObj)] call BIS_fnc_findSafePos;
	...
	_randomPos = [_cacheBuildingLocationFinal, 10, 300, 5, 0, 0.3, 0, [], (getPos sideObj)] call BIS_fnc_findSafePos;
	...
	for "_i" from 1 to 4 do {
		_randomPos = [getPos sideObj, 10, 250, 5, 0, 0.3, 0, [], (getPos sideObj)] call BIS_fnc_findSafePos;
...
if (alive sideObj) then {
    ...
    sleep (4 + (random 3));
    "SmallSecondary" createVehicle (_objPos getPos [random 1, random 360]);
    sleep 0.2;
    "SmallSecondary" createVehicle (_objPos getPos [random 1, random 360]);
    sleep (2 + (random 2));
...
//-------------------- DE-BRIEFING
	sideMissionUp = false;
	[nil, nil, _objectivepos] call AW_fnc_SMhintSUCCESS;
